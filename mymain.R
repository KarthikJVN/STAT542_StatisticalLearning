...
tslm_model <- function(train_ts, test_ts){
  ...
  for(j in 2:ncol(train_ts)){
    ...
    mo <- tslm(store_ts ~ season + trend)
    fcr <- forecast(mo, h=ho)
    test_ts[, j] <- as.numeric(fcr$mean)
  }
  test_ts
}


# Function writen to forecast using stlf (with arima and svd)
stlf_model<- function(train_ts, test_ts){
  ...
  for(j in 2:ncol(train_ts)){
    ho <- nrow(test_ts)
    store_ts <- ts(train_ts[, j], frequency=52)
    fcr <- stlf(store_ts, h=ho, s.window=7, method='arima', ic='bic')
    test_ts[, j] <- as.numeric(fcr$mean)
  }
  test_ts
}
...
## changes to the Model Fitting/Forecasting section
...
mypredict <- function() {
  ...
  for (dept in test_depts) {
    ...
    if (t < 7) {
      ...
      f_tslm <-  tslm_model(train_dept_ts, test_dept_ts)
      f_n <-  naive_model(train_dept_ts, test_dept_ts)
      test_month <- update_forecast(test_month, f_tslm, dept, 1)
      test_month <- update_forecast(test_month, f_tslm, dept, 2)
      test_month <- update_forecast(test_month, f_n, dept, 3)
      ...
      f_n <-  naive_model(train_dept_ts, test_dept_ts)
      test_month <- update_forecast(test_month, f_tslm, dept, 1)
      test_month <- update_forecast(test_month, f_stlf, dept, 2)
      test_month <- update_forecast(test_month, f_n, dept, 3)
    }
  }
